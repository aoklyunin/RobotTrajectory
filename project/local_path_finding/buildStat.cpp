...
std::vector<std::shared_ptr<LocalPathFinder>> pfs;


void testPath(std::vector<double> &start, std::vector<double> &end, int testNum) {
    info_msg("test begin");
    int i = 1;
    for (auto & pf:pfs) {
        ...
        std::vector<std::vector<double>> path = pf->findPath(start, end, errorCode);
        std::vector<std::vector<double>> findingPoses = pf->getFindingPoses();
...
//    SceneWrapper::dispState(path.front(),"path.front()");
        ...
        //assert(pf->checkPath(path) == PathFinder::NO_ERROR);

        //info_msg("path is valid");

        info_msg(pf->getCalculationTimeInSeconds(), " seconds");

        assert (errorCode == PathFinder::NO_ERROR);


        Json::Value result;
        result["path"] = pf->getJSONRepresentation(path);
        result["findingPoint"] = pf->getJSONRepresentation(findingPoses);
        result["time"] = pf->getCalculationTimeInSeconds();
        result["num"] = i;

        std::ofstream myfile;
        char buf[256];
        sprintf(buf,"testnum_%d_pfnum_%d",testNum,i++);
        std::string s = "../../../config/reports/buildStat_";
        s = s+buf+".json";
        myfile.open(s);
...
int main() {
    ...
    pfs.emplace_back(
            std::make_shared<SyncAStarPathFinder>(sceneWrapper, false, 1000, 10, 3000, 200, 5, 0, 0, 0, 8)
    );
    pfs.emplace_back(
            std::make_shared<SyncAStarPathFinder>(sceneWrapper, false, 1000, 10, 3000, 200, 5, 0, 0, 2, 8)
    );
    pfs.emplace_back(
            std::make_shared<SyncAStarPathFinder>(sceneWrapper, false, 1000, 10, 3000, 200, 2, 0, 0, 5, 8)
    );
    pfs.emplace_back(
            std::make_shared<SyncAStarPathFinder>(sceneWrapper, false, 1000, 10, 3000, 200, 5, 1 , 0, 0, 8)
    );

    info_msg("test sync speed tra finder");
